...
const ProjectCard = ({
  ...
  return <Card className={`overflow-hidden card-hover ${expanded ? 'shadow-md' : ''}`}>
      <CardHeader className="p-4 cursor-pointer" onClick={toggleExpand}>
        <div className="flex justify-between items-center">
          <CardTitle className="text-xl">{project.title}</CardTitle>
          <div className="flex items-center gap-2">
            {isProjectReserved && <Badge variant="secondary" className="bg-blue-100 text-blue-800 flex items-center gap-1">
        ...
        {!expanded && <CardDescription className="flex items-center text-sm gap-2 mt-2">
            <User size={14} />
      ...
      {expanded && <>
          ...
          <CardFooter className="flex justify-between p-4 pt-2 gap-2">
            {isStudent && <>
                {userProposal ? <Badge className={`mr-auto ${userProposal.status === 'approved' ? 'bg-green-100 text-green-800 hover:bg-green-200' : userProposal.status === 'rejected' ? 'bg-red-100 text-red-800 hover:bg-red-200' : userProposal.status === 'selected' ? 'bg-blue-100 text-blue-800 hover:bg-blue-200' : 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200'}`}>
                  ...
                  </Badge> : wasRejectedBefore ? <Badge className="mr-auto bg-red-100 text-red-800">
                    تم رفض مقترحك سابقًا
                  </Badge> : hasSelectedFinalProject ? <Badge className="mr-auto bg-blue-100 text-blue-800">
                    لديك مشروع نهائي بالفعل
                  </Badge> : isProjectReserved ? <Badge className="mr-auto bg-blue-100 text-blue-700 mx-0 px-[14px] rounded-full py-[6px]">
                    المشروع محجوز من طالب آخر
                  </Badge> : <Button onClick={() => setShowProposalForm(true)} className="ml-auto" disabled={userProposals && userProposals.length >= 3 || hasSelectedFinalProject || wasRejectedBefore || isProjectReserved}>
                    {userProposals && userProposals.length >= 3 ? 'الحد الأقصى للمقترحات (3)' : hasSelectedFinalProject ? 'لديك مشروع نهائي بالفعل' : wasRejectedBefore ? 'تم رفض مقترحك سابقًا' : isProjectReserved ? 'المشروع محجوز من طالب آخر' : 'تقديم مقترح'}
                  </Button>}
              </>}
      ...
      {showProposalForm && <ProposalForm project={project} onSubmit={handleSubmitProposal} onCancel={() => setShowProposalForm(false)} />}
    </Card>;
};
